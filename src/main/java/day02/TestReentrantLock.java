...
public class TestReentrantLock {
    static final MyReentrantLock LOCK = new MyReentrantLock(true);//true:公平锁    false:非公平锁
...
//        owner[null]   state[0]    当前获取锁的对象    加锁状态 0 表示没有对象获取这个锁，有的话是1 还有就是2 可重入锁
//        blocked queue     阻塞队列，获取锁失败的线程进入这个队列等待。双向链表。比如t1获取到了锁，则下t2,t3,t4进入到这个队列中，当前线程释放锁后，后面按照队列中顺序前去拿锁
//        waiting queue [c1]    对应条件变量，作为线程同步，自己创建的， static Condition c1 = LOCK.newCondition("c1");
//                              作用：获得锁之后，条件不满足，释放锁，当前线程存入到这个队列来等待
//        waiting queue [c2]    static Condition c2 = LOCK.newCondition("c2");
       /* new MyThread(() -> {
            LOCK.lock();
    ...
    private static void fairVsUnfair() throws InterruptedException {
        ...
        while (!stop) {
            new Thread(() -> {
                try {
                    boolean b = LOCK.tryLock(10, TimeUnit.MILLISECONDS);// 10毫秒获取不到锁，就放弃返回false，这样避免不断一直创建锁
                    if (b) {
